...
function ddCtAnalysis(treat, ctrl) {
  ...
  // per-sample ΔCt values: each target well Cq minus the group's mean IC Cq
  const icMeanT = mean(tIcCq);
  const icMeanC = has(cIcCq) ? mean(cIcCq) : icMeanT;
  const dCtValuesT = tTgCq.map(cq => cq - icMeanT);
  const dCtValuesC = cTgCq.map(cq => cq - icMeanC);
  // per-sample relative expression vs control: 2^−(ΔCt_sample − ΔCt_ctrl_mean)
  const dCtCtrlMean = mean(dCtValuesC);
  const relExprT = dCtValuesT.map(d => Math.pow(2, -(d - dCtCtrlMean)));
  const relExprC = dCtValuesC.map(d => Math.pow(2, -(d - dCtCtrlMean)));
  return {
    icWells: tIc.icWells,
    icGene: (treat.settings && treat.settings.internalControlGene) || '内参基因',
    treat: { icCq: tIcCq, targetCq: tTgCq, dCt: dCtT, dCtValues: dCtValuesT, relExpr: relExprT },
    ctrl: { icCq: cIcCq, targetCq: cTgCq, dCt: dCtC, dCtValues: dCtValuesC, relExpr: relExprC },
