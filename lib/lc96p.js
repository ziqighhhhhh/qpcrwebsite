...
function parseLc96p(buffer, fileName) {
  ...
  const curves = [];
  const melt = {}; // melt curves: key "well|dye" -> {well, dye, points:[{t,f}]}
  for (const re of findAll(rdmlRoot, 'react')) {
    ...
    for (const data of children(re, 'data')) {
      ...
      // melting data points (mdp): temperature vs fluorescence
      const mpts = [];
      for (const mdp of children(data, 'mdp')) {
        const t = numOf(child(mdp, 'tmp'));
        const f = numOf(child(mdp, 'fluor'));
        if (t === null || f === null) continue;
        mpts.push({ t, f });
      }
      if (mpts.length) melt[well + '|' + dye] = { well, dye, points: mpts };
    }
  }
