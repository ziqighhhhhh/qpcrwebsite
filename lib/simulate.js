...
function buildControl(exp, opts) {
  ...
  const curves = exp.curves.map(c => {
    ...
    // must stay stable across groups — a real internal control does not lag.
    if (pos && !/yellow|vic|hex|cy5/i.test(c.dye)) points = scaleCurve(shiftCurve(c.points, deltaCt), scale);
    return { well: c.well, dye: c.dye, channel: c.channel, targetId: c.targetId, points, shifted: pos, call: pos ? 'Positive' : 'Negative' };
  });
