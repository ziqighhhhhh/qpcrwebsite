...
function buildControl(exp, opts) {
  ...
  return {
    ...
    wells,
    curves,
    // control inherits the same melting profile (simulated shift affects amplification only;
    // SYBR product Tm specificity is unchanged). Annotated as simulated in analysis.
    melt: exp.melt ? JSON.parse(JSON.stringify(exp.melt)) : {},
