...
// Coherent simulation: Ct shifted right by deltaCt, AND amplitude (plateau / melt peak)
// scaled by 2^-deltaCt with a visible floor (minScale) so curves stay visible.
function buildControl(exp, opts) {
  opts = opts || {};
  const deltaCt = opts.deltaCt || 8;
  const suffix = opts.suffix || 'Ctrl';
  const minScale = opts.minScale !== undefined ? opts.minScale : 0.2;
  const scale = Math.max(Math.pow(2, -deltaCt), minScale);
  const computed = exp.computed || {};

  const isPos = (c) => {
    const call = computed[c.well + '|' + c.dye] ? computed[c.well + '|' + c.dye]['11 Intermediate Call'] : null;
    return call === 'Positive';
  };
  // wells the user marked as internal-control gene: they must NOT lag in the
  // control group (a real housekeeping gene is stable across groups)
  const icWells = new Set((exp.settings && exp.settings.internalControlWells) || []);
  const scaleCurve = (points, s) => {
    const base = Math.min.apply(null, points.map(p => p.f));
    return points.map(p => ({ c: p.c, f: base + (p.f - base) * s }));
  };

  const curves = exp.curves.map(c => {
    const pos = isPos(c);
    let points = c.points;
    // shift only the target (SYBR) channel; the IC/probe channel (e.g. Yellow555)
    // and user-marked internal-control wells must stay stable across groups —
    // a real internal control does not lag.
    const isIcWell = icWells.has(c.well);
    if (pos && !isIcWell && !/yellow|vic|hex|cy5/i.test(c.dye)) points = scaleCurve(shiftCurve(c.points, deltaCt), scale);
    return { well: c.well, dye: c.dye, channel: c.channel, targetId: c.targetId, points, shifted: pos && !isIcWell, call: pos ? 'Positive' : 'Negative' };
  });
  // melt: scale amplitude for positive SYBR wells (Tm position unchanged); IC has no melt
  const melt = {};
  if (exp.melt) {
    for (const key of Object.keys(exp.melt)) {
      const m = exp.melt[key];
      const pos = (computed[m.well + '|' + m.dye] ? computed[m.well + '|' + m.dye]['11 Intermediate Call'] : null) === 'Positive';
      melt[key] = pos
        ? { well: m.well, dye: m.dye, points: scaleCurve(m.points, scale) }
        : { well: m.well, dye: m.dye, points: m.points.map(p => ({ t: p.t, f: p.f })) };
    }
  }
  ...
  return {
    ...
    analysis: {
      ...
      simulated: {
        from: exp.name, deltaCt, scale: Math.round(scale * 100) / 100,
        method: 'positive wells shifted right by ' + deltaCt + ' cycles; plateau & melt amplitude scaled by ' + (Math.round(scale * 1000) / 1000) + ' (floor ' + minScale + ') to stay visible; negatives unchanged',
      },
    },
  };
}
...
function toLc96p(exp, sourceZip) {
  ...
  const reactsXml = exp.wells.map(w => {
    ...
    const blocks = exp.dyes.map(dye => {
      ...
      // melting points (mdp) if the model carries them (control inherits treatment melt)
      let mdps = '';
      const melt = exp.melt && exp.melt[w.id + '|' + dye];
      if (melt && melt.points && melt.points.length) {
        mdps = melt.points.map(mp =>
          '      <mdp><tmp>' + num(mp.t) + '</tmp><fluor>' + num(mp.f) + '</fluor></mdp>').join('\n');
      }
