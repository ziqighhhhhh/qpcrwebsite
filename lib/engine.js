...
// Curves are grouped by dye: SYBR dyes -> Sybr-KA.adf, probe dyes -> Gen-KA.adf
// (mirrors the original LC96 software's per-dye algorithm selection).
async function analyzeExperiment(exp) {
  const byAdf = {};
  for (const c of exp.curves) {
    const adf = /sybr/i.test(c.dye) ? 'Sybr-KA.adf' : 'Gen-KA.adf';
    (byAdf[adf] = byAdf[adf] || []).push(c);
  }
  const computed = {};
  for (const adf of Object.keys(byAdf)) {
    const inputs = byAdf[adf].map(c => ({
