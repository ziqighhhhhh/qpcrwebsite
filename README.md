...
- **图表（均内参归一）**：
  - 相对表达量柱状图：对照=1、实验=ΔΔCt 倍数，并叠加**每样本相对表达量散点**（2^−(ΔCt_样本−ΔCt_对照均值)），Y 轴范围自动纳入散点
  - Cq 分布图：显示**每孔 ΔCt（目的基因 Cq − 内参基因 Cq）**散点 + 箱线（实验组 vs 对照组），标题标注内参基因名
  - 统计卡片显示处理/对照 ΔCt 与内参稳定性（Cq 均值±SD）、ΔΔCt 明细表底部附归一化汇总行（CSV 导出含 IC 信息）
